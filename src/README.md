...
- **데이터 수집**: 익명 검색 키워드만 기록 — 사용자 식별 정보, 파일 내용, 개인정보는 일절 수집하지 않음
- **OAuth 범위**: Drive 읽기 권한만 요청 — 스크립트가 Drive 파일을 작성하거나 수정하는 동작은 없음
- **최초 실행 시**: Google에서 "확인되지 않은 앱" 경고가 표시될 수 있음 — 이는 자체 배포된 Apps Script 프로젝트에서 정상적으로 나타나는 현상이며 "고급 → `프로젝트 이름`(으)로 이동"을 클릭해 계속 진행할 수 있음
- **할당량 관리**: 6시간 캐싱 레이어가 Drive API 호출 횟수를 제한하여 일반적인 사용 환경에서 Google 무료 한도 내에서 운영 가능
...
| 항목 | 실질적 한도 | 비고 |
|------|-----------|------|
| 동시접속 | **30명** | Google Apps Script 동시 실행 제한 |
| 쿼리 내 키워드 수 | **5개 이하 권장** | 초과 시에도 동작하나 응답 지연 발생 |
| 활성 캐시 키워드 | **100개 자동 유지** | `PRECACHE_TOP_N` 설정값 기준, warmCache가 하루 4회 갱신 |
| 인덱싱 파일 수 | **10만 개 이하 권장** | 초과 시에도 이어하기 메커니즘으로 처리 가능 |
