...
파일 **내용**을 읽을 수는 없습니다. Drive 파일의 소유자는 스크립트 소유자이고,
...
| 제한 | 내용 |
|---|---|
| 순수 부정 질의 차단 | `NOT 면접`처럼 제외 조건만 있는 질의는 거부합니다. `논술 NOT 면접`은 정상 동작합니다. |
| 최소 키워드 길이 | 2글자. 한 글자는 부분 문자열 매칭이라 사실상 전 파일에 걸립니다. |

> 이 제한은 정책 일관성을 위한 것이지 열거 방지 수단이 아닙니다. 두 글자 질의
> 하나로도 대부분의 파일이 반환됩니다. 파일 목록이 공개되면 안 된다면 배포 설정의
> **액세스 권한이 있는 사용자**를 학교 Workspace 도메인 내로 제한하세요.
