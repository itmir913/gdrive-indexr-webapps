...
구글 드라이브 공유 폴더에 저장된 입시자료 PDF를 키워드로 빠르게 검색할 수 있는 Google Apps Script 웹앱입니다.
외부 서버·회원가입 없이 Google Drive + Apps Script + Google Sheets만으로 동작합니다.

## 주요 기능

- **파일명 + PDF 내부 텍스트** 동시 검색 (Drive API v3 `fullText contains` 활용)
- **Boolean 연산자** 지원: `AND`(공백), `OR`, `NOT`, `(` `)` 괄호 그룹
- **6시간 캐싱** (CacheService) — 동일 키워드 재검색 시 Drive API 호출 없이 즉시 반환
- **하루 1회 인덱스 자동 갱신** (02:00 트리거) + **하루 4회 캐시 워밍** (02:30 / 07:30 / 12:30 / 17:30)
- **키워드 빈도 로그** — 검색어를 익명으로 수집해 캐시 워밍 우선순위에 활용 (30일 미검색 키워드는 매일 03:00 자동 삭제)
...
스케줄 트리거
  ...
  03:00  purgeStaleKeywords   — 30일 미검색 키워드 KeywordLog에서 삭제
  07:30  warmCache
  12:30  warmCache
  17:30  warmCache
```
...
검색 대상 루트 폴더를 열고 URL에서 폴더 ID를 복사합니다.
```
...
2. 기본 `Code.gs` 내용을 모두 지우고 이 저장소의 `Code.gs` 를 붙여넣습니다.
3. `+` 버튼으로 HTML 파일을 추가하고 이름을 **`index`** 로 지정한 뒤 `index.html` 내용을 붙여넣습니다.

### 4단계 — 상수 값 교체

`Code.gs` 상단의 두 상수를 실제 값으로 교체합니다.
...
Apps Script 편집기에서 함수를 선택해 수동으로 실행합니다.

```
rebuildMetadataIndex  실행 → FileIndex 시트에 PDF 목록이 채워지는지 확인
```
...
```
setupTriggers  실행 → Apps Script 편집기 → 트리거 메뉴에서 6개 확인
```
...
3. 설정:
   - 실행 계정: **나 (스크립트 소유자)**
   - 액세스 권한: **도메인 내 모든 사용자** (Google Workspace for Education 도메인)
