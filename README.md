...
- **파일명 + 모든 파일 내부 텍스트 동시 검색** 동시 검색 (Drive API v3 `fullText contains` 활용)
...
> 만약 파일 검색 웹앱 자체를 Google 로그인한 사람만 접근할 수 있도록 설정하려면, `엑세스 권한이 있는 사용자`를 **Google 계정이 있는 모든 사용자**로 설정하면 됩니다.
