...
- **키워드 빈도 로그** — 검색어를 익명으로 수집해 캐시 워밍 우선순위에 활용
...
스케줄 트리거
  ...
  02:30  warmCache            — 상위 30개 키워드 캐시 사전 워밍
  07:30  warmCache
  12:30  warmCache
  17:30  warmCache
```
...
```
setupTriggers  실행 → Apps Script 편집기 → 트리거 메뉴에서 5개 확인
```
...
| `PRECACHE_TOP_N` | `30` | 캐시 워밍 대상 상위 키워드 수 |
