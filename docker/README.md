...
> 스케줄은 `TZ` 기준으로 동작합니다. 백엔드는 `TZ` 가 없으면 `Asia/Seoul` 을 기본값으로
...
```json
{
  ...
  "isIndexing": false,
  "indexedCount": 1024,
  "lastRebuild":   { "at": "2026-09-18 02:04:11", "result": "done", "message": null },
  "lastDriveError": { "at": null, "message": null }
}
```

`status` 는 마지막 재빌드가 실패했으면 `degraded` 가 됩니다. `authenticated` 는
`token.json` 에 `refresh_token` 이 있는지만 보므로, 토큰이 폐기됐거나 영구 403 상태여도
`true` 입니다. **갱신이 조용히 멈춘 상태는 `status` 와 `lastRebuild` 로 판단하세요.**
관리자 모달에도 같은 정보가 표시됩니다.

---

## 접근 권한과 노출 범위

노출 범위는 **두 가지 설정의 조합**으로 정해집니다. 하나만 잠가서는 의도한 결과가 나오지 않습니다.

| 축 | 설정 위치 | 통제 대상 |
|---|---|---|
| ① 웹앱 접근 권한 | nginx 앞단 (**기본값: 인증 없음**) | **검색**할 수 있는 사람 |
| ② Drive 폴더 공유 | 루트 폴더 우클릭 → **공유** | **파일을 열 수** 있는 사람 |

백엔드는 OAuth를 승인한 계정의 권한으로 Drive를 조회합니다. `fullText` 질의는 그 계정이 볼
수 있는 드라이브 전체를 훑지만, **응답은 색인된 폴더(`FOLDER_ID` 하위)에 있는 파일로
걸러집니다.** 따라서 `/api/search` 에 닿을 수 있는 사람이 검색할 수 있는 범위는
**색인 대상 폴더**입니다. 반면 결과 링크를 눌러 파일을 여는 것은 그 사람 본인의 권한이므로
②가 결정합니다. **두 축은 독립입니다.**

기본 구성에는 ①이 없습니다. `/api/search` 는 무인증이고 응답에 파일명·폴더 경로·Drive
링크가 담깁니다. 공개 도메인에 올린다면 **파일 목록은 사실상 공개 정보**입니다.

### 조합별 결과

| ① 웹앱 접근 | ② 폴더 공유 | 결과 |
|---|---|---|
| 없음 (기본값) | 링크가 있는 모든 사용자 | **완전 공개.** URL을 아는 누구나 검색하고 파일도 연다 |
| 없음 (기본값) | 지정 계정만 | URL을 아는 누구나 **검색**은 된다(아래 주의). 파일 열람은 막힌다 |
| basic auth / IP 허용 목록 / Cloudflare Access | 도메인 내 공유 | **내부 전용.** 허용된 사람만 검색·열람 — 학교 배포에 권장 |

> **①만 열고 ②를 잠가도 내용이 새는 경로가 있습니다.** 검색의 절반은 Drive `fullText`
> 질의입니다. 어떤 문구가 파일 **본문에** 들어 있는지가 검색 결과로 드러나므로, 파일을
> 열지 못하는 사람도 `"홍길동 3학년"` 같은 문구로 그런 내용을 담은 파일의 존재를 확인할
> 수 있습니다. 개인정보가 담긴 문서가 검색 대상 폴더에 섞여 있다면 **②가 아니라 ①부터
> 제한해야 합니다.**
>
> 이 오라클의 범위도 아래와 같이 **색인된 폴더 안**으로 한정됩니다.

### ① 웹앱 접근 제어를 추가하려면

`nginx.conf` 의 `location /api/` 에 basic auth를 걸거나, `allow`/`deny` 로 IP를 제한하거나,
Cloudflare Access 같은 앞단 인증을 둡니다. `/oauth/` 경로는 Google 콜백이 도달해야 하므로
함께 막지 않도록 주의하세요.
...
| 제한 | 내용 |
|---|---|
| 순수 부정 질의 차단 | `NOT 면접`, `NOT A OR B`처럼 제외 조건만 있는 질의는 거부합니다. `논술 NOT 면접`과 이중 부정(`NOT NOT 논술`)은 정상 동작합니다. |
| 최소 키워드 길이 | 2글자. 한 글자는 부분 문자열 매칭이라 사실상 전 파일에 걸립니다. |
| Drive 호출 예산 | `files.list` 호출 기준 분당 200회. 캐시로 처리되는 검색은 예산을 쓰지도, 예산 소진으로 거부되지도 않습니다. Drive 호출이 실제로 필요한 검색만 429가 됩니다. |

> 위 제한은 **정책 일관성과 Drive 쿼터 보호를 위한 것이지 접근 제어가 아닙니다.**
> 부분 문자열 매칭 특성상 `20` 같은 두 글자 질의 하나로도 대부분의 파일이 반환됩니다.
> 노출 범위는 위의 ①·② 설정으로 정하세요.
...
| 스크립트 | 내용 |
|---|---|
| `npm run test:parser` | Boolean 파서·검색 키 단위 테스트 (92개) |
| `npm run test:parity` | **GAS ↔ Docker 구현 대조** (526개, 일반 루트 / 슬래시 든 루트 2세트) |

### 두 구현을 함께 고쳐야 하는 이유

이 저장소에는 같은 앱의 구현이 둘 있습니다 — `src/Code.gs`(Apps Script)와
`docker/backend/`(Node.js). 검색 동작의 핵심은 **양쪽에 따로 작성돼 있고 자동으로
동기화되지 않습니다.**

`test-parity.js`는 같은 질의·같은 픽스처로 **양쪽의 실제 진입 함수를 호출해** 비교합니다.
GAS는 `Code.gs`를 `vm`에 올려 진짜 `doSearch()`를, Docker는 `/api/search`가 쓰는 진짜
`runSearch()`를 부릅니다. 스텁은 외부 I/O(Drive 호출, 키워드 로깅)뿐이고,
`CacheService`·`SpreadsheetApp`은 인메모리 구현이라 `getCachedMetadataMap`과 청크 캐시가
실제로 동작합니다. 응답 형태(결과 배열 / `{error}`), 결과 집합, 로깅된 키워드를 모두 봅니다.

> 이전 판은 매칭 로직을 테스트 안에서 재구현해 비교했습니다. 그래서 실제 매칭 함수와
> 응답 형태가 대조 대상에서 빠져 있었고, 알려진 버그 두 개를 다시 넣어도 전부 통과했습니다.
> 테스트를 바꿀 때는 **버그를 일부러 주입해 실패하는지 먼저 확인하세요.**
...
| 검색 흐름 · 응답 형태 · 로깅 순서 | `backend/search-pipeline.js` | `Code.gs`의 `doSearch`, `_extractKeywords` |
...
SQLite 트랜잭션, OAuth·토큰 갱신, 크론 발화, LockService·ScriptProperties,
Drive API 응답, 프론트엔드 DOM. 이 영역은 실환경에서 확인해야 합니다.
