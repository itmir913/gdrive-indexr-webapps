...
백엔드는 OAuth를 승인한 계정의 권한으로 Drive를 조회합니다. 따라서 `/api/search` 에 닿을 수
있는 사람은 그 계정이 볼 수 있는 파일 전체를 검색할 수 있습니다. 반면 결과 링크를 눌러
파일을 여는 것은 그 사람 본인의 권한이므로 ②가 결정합니다. **두 축은 독립입니다.**
...
| `npm run test:parity` | **GAS ↔ Docker 구현 대조** (494개, 일반 루트 / 슬래시 든 루트 2세트) |
