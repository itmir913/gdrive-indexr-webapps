...
`/api/search` 에는 **인증이 없습니다.** URL을 아는 누구나 검색할 수 있고, 응답에는
파일명·폴더 경로·Drive 링크가 담깁니다. 즉 **파일 목록은 사실상 공개 정보**입니다.
GAS 버전도 같은 모델입니다(`src/README.md` 「접근 권한 구조」 참고).

파일 **내용**을 읽을 수는 없습니다. Drive 파일의 소유자는 OAuth를 승인한 계정이고,
외부인이 링크를 받아도 따로 공유돼 있지 않으면 열 수 없습니다.

> **다만 내용의 존재 여부는 확인할 수 있습니다.** 검색의 절반은 Drive `fullText` 질의라,
> 어떤 문구가 파일 **본문에** 들어 있는지가 검색 결과로 드러납니다. 즉 무인증 검색은
> 파일 내용에 대한 **오라클**입니다. 파일을 열어 읽을 수는 없지만
> `"홍길동 3학년"` 같은 문구를 질의해 그런 내용을 담은 파일이 있는지는 알아낼 수 있습니다.
>
> **개인정보가 담긴 문서가 검색 대상 폴더에 섞여 있다면 이 공개 모델은 적절하지 않습니다.**

검색 API에는 다음 제한이 있습니다.

| 제한 | 내용 |
|---|---|
| 순수 부정 질의 차단 | `NOT 면접`, `NOT A OR B`처럼 제외 조건만 있는 질의는 거부합니다. `논술 NOT 면접`은 정상 동작합니다. |
...
> 위 제한은 **정책 일관성과 Drive 쿼터 보호를 위한 것이지 열거 방지 수단이 아닙니다.**
> 부분 문자열 매칭 특성상 `20` 같은 두 글자 질의 하나로도 대부분의 파일이 반환됩니다.
> 파일 목록이 공개되면 안 되는 환경이라면 앞단에 접근 제어를 두세요 —
> nginx basic auth, IP 허용 목록, Cloudflare Access 등.
